...
elapsed_time = toc;   % остановка таймера
fprintf('Симуляция завершена за %.2f секунд.\n', elapsed_time);
...
if ~isempty(rpm_lines)
    ...
    % Исключаем уровень 800
    unique_bsfc = unique_bsfc(unique_bsfc ~= 800);
    ...
    for i = 1:length(unique_bsfc)
        ...
        if length(rpm_level) > 2
            ...
            % --- Выбор точки для подписи ---
            % Ищем точки с rpm > 1000 (без учёта замыкающей)
            idx_above_1000 = find(rpm_level(1:end-1) > 1000);
            if ~isempty(idx_above_1000)
                % Среди них выбираем точку с минимальным моментом
                [~, idx_min] = min(torque_level(idx_above_1000));
                idx_final = idx_above_1000(idx_min);
            else
                % Если нет точек выше 1000, берём точку с минимальным моментом вообще
                [~, idx_final] = min(torque_level(1:end-1));
            end
            % Подпись прямо на линии
            text(rpm_level(idx_final), torque_level(idx_final), num2str(level), ...
                'FontName', 'GOST Type A',   'FontAngle', 'Italic' , 'FontSize', 12, ...
                'BackgroundColor', 'w', 'EdgeColor', 'none', ...
                'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
                
        elseif length(rpm_level) == 2
            plot(rpm_level, torque_level, 'Color', colors(i, :), 'LineWidth', 1.5);
            % Для прямой линии подпись в середине (с проверкой на обороты)
            if mean(rpm_level) > 1000
                text(mean(rpm_level), mean(torque_level), num2str(level), ...
                    'FontName', 'GOST Type A',   'FontAngle', 'Italic' , 'FontSize', 12, ...
                    'BackgroundColor', 'w', 'EdgeColor', 'none', ...
                    'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
            else
                % Если средние обороты ниже 1000, ставим подпись в середине всё равно
                text(mean(rpm_level), mean(torque_level), num2str(level), ...
                    'FontName', 'GOST Type A',   'FontAngle', 'Italic' , 'FontSize', 12, ...
                    'BackgroundColor', 'w', 'EdgeColor', 'none', ...
                    'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
